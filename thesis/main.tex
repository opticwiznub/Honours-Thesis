...
The advent of deep learning allowed algorithms to progressively extract higher-level features from raw data.\cite{deng-deep-learning} The MLP 
...
\begin{equation}
f(\mathbf{A, X}) = \sigma_2( \mathbf{ \hat{A} }_2 \sigma_1 (\mathbf{ \hat{A} }_1 \mathbf{X} W^{(1)}) W^{(1)})
\end{equation}

\noindent where $\sigma_1(.)$ and $\sigma_2(.)$ are an activation function such as ReLU, $\hat{A} = \tilde{D}^{-1/2} (A + I) \tilde{D}^{-1/2}$
